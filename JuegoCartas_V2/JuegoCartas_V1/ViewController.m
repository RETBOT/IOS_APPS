...
- (IBAction)btnCarta:(UIButton *)sender {
    ...
    if([sender.currentTitle length]){
        [sender setBackgroundImage:
         [UIImage imageNamed:@"Back-card"] forState:UIControlStateNormal];
        [sender setTitle:@"" forState:UIControlStateNormal];
    } else {
        Carta *randomCard = [self.deck drawRandomCard];
        if (randomCard) {
            [sender setBackgroundImage:
             [UIImage imageNamed:@"Blank-card-Mexico"] forState:UIControlStateNormal];
            [sender setTitle:@"Mexico" forState:UIControlStateNormal];
        }
    }
    /*
...
-(UIImage *)backgroundImageForCard:(Carta *)card{
    return [UIImage imageNamed:card.isChosen ? @"" : @"Back-card"];
}
